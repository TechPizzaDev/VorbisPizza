...
internal sealed class Codebook
{
    ...
    private readonly byte _mapType;
    private readonly int[] _lengths;
    private readonly float[] _lookupTable;
    private readonly HuffmanListNode[] _overflowList;
    private readonly HuffmanListNode[] _prefixList;
    private readonly byte _prefixBitLength;
    private readonly ushort _maxBits;
    ...
    public Codebook(ref VorbisPacket packet)
    {
        ...
        // get the counts
        _dimensions = (ushort)packet.ReadBits(16);
        int entries = (int)packet.ReadBits(24);
        ...
        _mapType = (byte)packet.ReadBits(4);
        _lookupTable = InitLookupTable(ref packet, _mapType, entries, _dimensions);
    }

    private Huffman InitTree(ref VorbisPacket packet, out ushort maxBits)
    {
        bool sparse;
        int total = 0;
    ...
    private float[] InitLookupTable(ref VorbisPacket packet, int mapType, int entries, int dimensions)
    {
        if (mapType == 0)
        {
        ...
        if (mapType == 1)
        {
            lookupValueCount = lookup1_values(entries, dimensions);
        }
    ...
    {
        return _lookupTable.AsSpan(entry * _dimensions, _dimensions);
    }
    ...
    public ushort Dimensions => _dimensions;

    public int Entries => _lengths.Length;

    public byte MapType => _mapType;
}
