...
namespace NVorbis
{
    ...
    internal class Floor1 : IFloor
    {
        ...
        private abstract class Data : IFloorData
        {
            ...
            public abstract Span<int> PostSpan { get; }
        }

        private class MemData : Data
        {
            private int[] _posts;

            public override Span<int> PostSpan => _posts;

            public MemData(ReadOnlySpan<int> source)
            {
                _posts = source.ToArray();
                PostCount = source.Length;
            }
        }
            ...
            where T : unmanaged
        {
            ...
            public Data(ReadOnlySpan<int> source)
            {
                source.CopyTo(PostSpan);
                PostCount = source.Length;
            }

            public override Span<int> PostSpan => MemoryMarshal.Cast<T, int>(
                MemoryMarshal.CreateSpan(ref _posts, Unsafe.SizeOf<T>() / sizeof(int)));
        }

        [StructLayout(LayoutKind.Explicit, Pack = 1, Size = 0 * sizeof(int))]
        internal struct PostList0
        {
        }

        [StructLayout(LayoutKind.Explicit, Pack = 1, Size = 8 * sizeof(int))]
        internal struct PostList8
        {
        }

        [StructLayout(LayoutKind.Explicit, Pack = 1, Size = 16 * sizeof(int))]
        internal struct PostList16
        {
        }

        [StructLayout(LayoutKind.Explicit, Pack = 1, Size = 24 * sizeof(int))]
        internal struct PostList24
        {
        }

        [StructLayout(LayoutKind.Explicit, Pack = 1, Size = 32 * sizeof(int))]
        internal struct PostList32
        {
        }

        [StructLayout(LayoutKind.Explicit, Pack = 1, Size = 40 * sizeof(int))]
        internal struct PostList40
        {
        }

        [StructLayout(LayoutKind.Explicit, Pack = 1, Size = 48 * sizeof(int))]
        internal struct PostList48
        {
        }

        [StructLayout(LayoutKind.Explicit, Pack = 1, Size = 56 * sizeof(int))]
        internal struct PostList56
        {
        }

        [StructLayout(LayoutKind.Explicit, Pack = 1, Size = 64 * sizeof(int))]
        internal struct PostList64
        {
        }

        #endregion

        private static int[] RangeLookup { get; } = { 256, 128, 86, 64 };
        private static int[] YBitsLookup { get; } = { 8, 7, 7, 6 };
        ...
        public IFloorData Unpack(IPacket packet, int blockSize, int channel)
        {
            ...
            if (!packet.ReadBit())
                return new Data<PostList0>(default);

            Span<int> posts = stackalloc int[Data.MaxCount];
            int postCount = 0;

            posts[postCount++] = (int)packet.ReadBits(_yBits);
            posts[postCount++] = (int)packet.ReadBits(_yBits);
            ...
            for (int i = 0; i < _partitionClass.Length; i++)
            {
                ...
                for (int j = 0; j < cdim; j++)
                {
                    ...
                    {
                        if ((posts[postCount] = book.DecodeScalar(packet)) == -1)
                        {
                            // we read a bad value... bail
                            return new Data<PostList0>(default);
                        }
                        ...
                        if (postCount > 32)
                        {
                            if (postCount > 40)
                            {
                                if (postCount > 48)
                                {
                                    if (postCount > 56)
                                    {
                                        return new Data<PostList64>(posts);
                                    }
                                    return new Data<PostList56>(posts);
                                }
                                return new Data<PostList48>(posts);
                            }
                            return new Data<PostList40>(posts);
                        }
                        return new Data<PostList32>(posts);
                    }
                    return new Data<PostList24>(posts);
                }
                return new Data<PostList16>(posts);
            }
            return new Data<PostList8>(posts);
        }
        ...
        public void Apply(IFloorData floorData, int blockSize, float[] residue)
        {
            ...
            Span<int> posts = data.PostSpan.Slice(0, data.PostCount);

            int lx = 0;
            int ly = posts[0] * _multiplier;
        ...
        private void UnwrapPosts(Data data, Span<bool> stepFlags)
        {
            stepFlags[0] = true;
            stepFlags[1] = true;

            var posts = data.PostSpan.Slice(0, data.PostCount);
            ...
            for (int i = 2; i < posts.Length; i++)
            {
                ...
                int predicted = RenderPoint(
                    _xList[lowOfs], finalY[lowOfs], _xList[highOfs], finalY[highOfs], _xList[i]);
